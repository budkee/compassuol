...
select count(*)
from sales.funnel -- 30580 visitas

-- (Exemplo 2) Contagem das linhas de uma coluna
-- Conte todos os pagamentos registrados na tabela sales.funnel 

select count(paid_date)
from sales.funnel -- 3788 vendas

-- (Exemplo 3) Contagem distinta de uma coluna
-- Conte todos os produtos distintos visitados em jan/21

select distinct count(product_id) -- (1058) não reconhece os distintos e recolhe todas as linhas
select count(distinct product_id) -- (139) reconhece apenas os distintos
from sales.funnel
